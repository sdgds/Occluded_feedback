...
在随机Hopfield神经网络中，神经元的激活概率由以下sigmoid函数决定： $$P(s_i = +1 \mid h_i) = \frac{1}{1 + e^{-\beta \cdot h_i}}$$ 其中：
$s_i$：第i个神经元的状态（+1或-1）
$h_i$：局部场强（来自其他神经元的输入）
...
2.3 竞争机制
Face和Place两个区域通过反相调节各自的β值实现对抗性竞争：
...
4.2 第一步：计算激活统计量
4.2.1 平均激活率 r
$$r = \frac{1}{\tau} \sum_{i=1}^{\tau} \text{firing_rate}(t-i)$$ 代码实现：
...
4.2.2 一阶时间差分 r₁
$$r_1 = \frac{1}{\tau} \sum_{i=1}^{\tau} \left[ \text{firing_rate}(t-i) - \text{firing_rate}(t-i-1) \right]$$ 代码实现：
...
触发条件： $$r > 0.78 + \mathcal{N}(0, 0.09)$$ 计算公式： $$\beta(r) = \frac{15 - 2}{1 + e^{25(r - 0.5)}} + 2 + \mathcal{N}(0, 1.1)$$ 代码实现：
...
触发条件： $$r \leq 0.78 + \mathcal{N}(0, 0.09)$$
5.2.1 计算对手区域激活
...
数学表达： $$r_{\text{opponent}} = \frac{1}{\tau} \sum_{i=1}^{\tau} \text{firing_rate}_{\text{opponent}}(t-i)$$
5.2.2 关键切换判断
这是产生Gamma分布感知持续时间的核心机制。
...
数学表达： $$ \beta_{\text{current}} = \begin{cases} 15 + \mathcal{N}(0, 1.6) & \text{if } r_{\text{opponent}} < 0.8 + \mathcal{U}(-0.1, 0.05) \ 2 + \mathcal{N}(0, 0.9) & \text{otherwise} \end{cases} $$ 物理意义分析：
对手状态	opponent_r	当前区域β	结果
对手主导	> 0.8	2 ± 0.9	当前区域获得低β（高增益），准备夺回主导权
对手未主导	< 0.8	15 ± 1.6	当前区域维持高β（高噪声），继续被抑制
关键创新：
np.random.uniform(-0.1, 0.05) 产生不对称的随机阈值
阈值范围：[0.7, 0.85]
...
7.3 Sigmoid函数参数
完整表达式： $$\beta(r) = \frac{\beta_{\max} - \beta_{\min}}{1 + e^{k(r - r_0)}} + \beta_{\min} + \epsilon$$
参数符号	代码中的值	物理意义
...
8.2 关键时刻详解（t=20）
Face区域的计算：
Face的 $r = 0.75 < 0.78$ → 进入"对手抑制"分支
查询Place的激活：$r_{\text{opponent}} = 0.78$
生成随机阈值：$\text{threshold} = 0.8 + \mathcal{U}(-0.1, 0.05) = 0.72$（假设）
判断：$0.78 > 0.72$ → Face获得 $\beta = 2$（准备让出主导）
Place区域的计算：
Place的 $r = 0.78 > 0.78$ → 触发自适应分支
Place获得 $\beta \approx 12$（适度抑制，但仍在主导）
系统行为：
...
多个随机判断的叠加效应
9.2 从随机过程到Gamma分布
数学原理： 假设感知状态的切换由泊松过程驱动，但切换率 $\lambda$ 本身是随机的： $$\lambda(t) = \lambda_0 \cdot f(r(t), r_{\text{opponent}}(t), \text{noise})$$ 当切换率具有随机性时，等待时间分布从指数分布演化为Gamma分布： $$P(T = t) = \frac{\lambda^k}{\Gamma(k)} t^{k-1} e^{-\lambda t}$$ 其中：
$k$：形状参数（与噪声强度相关）
...
Levelt, W. J. (1967). Note on the distribution of dominance times in binocular rivalry. British Journal of Psychology, 58(1‐2), 143-145.
